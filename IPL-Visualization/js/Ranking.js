...
class Ranking{
	constructor(ranking_data){
		...
		this.svg = d3.select(".content").append("svg").attr("width", width + margin.left + margin.right).attr("height", height + margin.top + margin.bottom).append("g").attr("transform", "translate(" + margin.left + "," + margin.top + ")");
		...
		this.svg.append("defs").append("clipPath").attr("id", "clip").append("rect").attr("width", width).attr("height", height + cfg.strokeWidth);
		this.xGroup = this.svg.append("g");
		this.xGroup.append("g").attr("transform", "translate(" + [0, height + axisMargin * 1.2] + ")").attr("class", "x-axis")
    ...
    // 		height = 
   	...
   	// var this.svg = d3.select("body").append("this.svg")
    ...
    //var this.colour = d3.scaleOrdinal(d3.schemeAccent);
    

    //var this.highlight = ["Rajasthan Royals", "Punjab Kings", "Chennai Super Kings", "Delhi Capitals", "Mumbai Indians", "Kolkata Knight Riders", "Royal Challengers Banglore", "Hyderabad Deccan Chargers", "Gujurat Titans", "Lucknow Super Giants"];
    ...
    //   this.colour.domain(data.map(d => d.department));
    ...
    //   var endLabels = this.svg.append("g")
