...
class Ranking{
	constructor(ranking_data){
		...
		this.svg = d3.select(".content").append("svg").attr('id','ranking').attr("width", width + margin.left + margin.right).attr("height", height + margin.top + margin.bottom).append("g").attr("transform", "translate(" + margin.left + "," + margin.top + ")");
	...
	drawRankingGraph(team1,team2){
		...
		console.log(team1);
		console.log(team2);
		d3.select("#ranking").select("text").text("Ranking History").attr('x',width/2).attr('y',height).style("font-weight","bold");
